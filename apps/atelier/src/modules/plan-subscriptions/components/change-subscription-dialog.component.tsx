...
export const ChangeSubscriptionDialog = ({ availablePaymentMethods }: ChangeSubscriptionDialogProps) => {
  const { planSelected, setPlanSelected, priceSelected, setPriceSelected } = UsePlanSubscription()
  ...
  const forceCheckout = useRef(false);
  const router = useRouter();
  const { result, errors, isPending, cleanErrors, cleanResult, handleSubmit } = useHandleAction({
    action: initiateSubscriptionAction,
    beforeAction: async () => {
      //do something
    },
    afterAction: async (result) => {
      if (result.data?.ok && result.data.redirect_url) {
  ...
  const submitSubscription = () => {
    ...
    formData.set('success_url', config.app_url + '/get-started/success');
    formData.set('cancel_url', config.app_url + '/get-started/failed');
    if (forceCheckout.current) {
...
if (isPending) {
  ...
  if (result || errors) {
    ...
    const canRetry = result?.data?.stripe_error?.isRetryable;

    return (
      <Dialog open={true}>
        <DialogContent className="w-full max-w-md p-8">
          <div className="flex flex-col items-center justify-center gap-6 text-center">
            <DialogTitle className={cn("text-2xl font-semibold", {
              "text-green-500": success,
              "text-red-500": !success
            })}>
              {success ? "Subscription Updated!" : "Something went wrong"}
            </DialogTitle>
            <p className="text-text-muted">{message}</p>
            <div className="flex flex-col gap-3 w-full">
              {canRetry? <form onSubmit={handleRetry}>
