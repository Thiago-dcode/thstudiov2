...
export const userSessionByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
...
export const setRememberMeByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
...
export const getRememberMeByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
...
export const deleteRememberMeByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
...
export const setUserSessionByCookie = async (userAuth: UserAuth,cookies:RequestCookies|ResponseCookies)=>{
    const expireAt = addDays(new Date(), 1);
    const cookieOptions = {
        httpOnly: true,
        expires: expireAt
    }
    cookies.set(USER_AUTH_COOKIE_NAME, encrypt(JSON.stringify(userAuth), SECRET), cookieOptions);
...
export const getUserSessionExpirationDateByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
    const cookieValue = cookies.get(USER_AUTH_COOKIE_EXPIRATION_DATE)?.value ?? null;
    if(!cookieValue){
        return null;
    }
...
export const deleteUserSessionByCookie = async (cookies:RequestCookies|ResponseCookies)=>{
