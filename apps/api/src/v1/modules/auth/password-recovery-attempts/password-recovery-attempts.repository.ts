...
export class PasswordRecoveryAttemptsRepository extends BaseRepository {
  private readonly BASE_COLUMNS: PasswordRecoveryAttemptColumns[] = [
    ...
    'password_recovery_attempts.user_auth_device_id',
  ...
  async findOneById(id: number) {
    return await this.queryBuilder.where('id', '=', id).select(this.COLUMNS_WITH_USER).join('user_id', 'users', 'id', 'LEFT').first<PasswordRecoveryAttemptSchemaWithUser>();
  }
  ...
  async findNotExpiredByUserId (userId: number) {
    return await this.queryBuilder.where('user_id', '=', userId).where('expires_at', '>', new Date()).select(this.BASE_COLUMNS).first<PasswordRecoveryAttemptSchema>();
  }
  async findOneByCode(code: string) {
    return await this.queryBuilder.where('code', '=', code).select(this.COLUMNS_WITH_USER).join('user_id', 'users', 'id', 'LEFT').first<PasswordRecoveryAttemptSchemaWithUser>();
  }
