        ...
        <section className="w-full flex flex-col gap-3 items-start justify-start">
            ...
            <div className="flex items-stretch justify-start gap-3 w-full flex-wrap">
                {Array.from({ length: 6 }).map((_, i) => (
                    <div
                        key={`skeleton-${i}`}
                        className="flex flex-col items-center justify-center gap-2 min-w-30 rounded-lg p-3 bg-fg border border-border"
                    >
                ...
                {METRICS.map(({ title, used, limit, format }, i) => {
                    ...
                    return (
                        <div
                            key={`${title}-${i}`}
                            className={cn(
                                "flex flex-col items-center justify-between gap-2 min-w-30 rounded-lg px-3 py-2.5",
                                "bg-fg border transition-colors",
                                styles.border,
                            )}
                        >
                            <p className="text-xs font-medium text-text-muted tracking-wide uppercase">{title}</p>
                            <p className={cn("font-semibold text-base tabular-nums", styles.label)}>{displayValue}</p>

                            {limit !== -1 && (
                                <div className={cn("w-full h-1.5 rounded-full overflow-hidden", styles.track)}>
                                    <div
