...
export default async function UpdateSubscriptionPage() {
    ...
    const filteredPlans = plans.data.filter(p => !p.is_free && (!activePlan?.data?.base_price || p.base_price > activePlan?.data?.base_price));

    //User already on the more expensive plan
    if (filteredPlans.length === 0) {
        const currentPlanName = activePlan?.data?.translation?.name || activePlan?.data?.name || 'your current plan';
        return (
            <AdminPageContainer>
                ...
                <div className="flex items-center justify-center pt-8">
                    <section className="w-full max-w-lg border border-fg-2 rounded-md px-6 py-6 space-y-2">
                        <p className="text-base font-semibold">You're already on the most powerful plan.</p>
                        <p className="text-sm text-text-muted">
                            Your current plan is <span className="font-medium text-text">{currentPlanName}</span> — there’s nothing higher to upgrade to right now.
                        </p>
                        <div className="pt-2">
