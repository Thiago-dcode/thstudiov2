...
export default async function PortfolioCreate() {
    ...
    const limitInfo = getResourceLimitInfo(metricsResponse.data ?? null, TABLES_ENUM.PORTFOLIOS);
    if (limitInfo?.isAtLimit) {
        return (
            <AdminPageContainer>
                <AdminPageTitle title="Create a portfolio" />
                <ResourceLimitReached label={limitInfo.label} backHref="/atelier/portfolios" count={limitInfo.count} limit={limitInfo.limit} />
            </AdminPageContainer>
        );
    }
