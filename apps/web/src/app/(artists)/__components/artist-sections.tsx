...
export const ArtistSections = async ({ username, userId }: ArtistSectionsProps) => {
    const [portfolioRes, collectionRes, serviceRes] = await Promise.all([
        portfolioService.findAll({ user_id: userId, paginated: false, is_highlight: true }),
    ...
    return (
        <div className="w-full max-w-5xl mx-auto px-6 md:px-12 space-y-28">
            {portfolios.length > 0 && (
                <section>
                    <SectionHeader
                        title="Portfolios"
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 tablet:grid-cols-4 tablet:gap-5">
                        {collections.map((collection) => (
                            ...
                            >
                                <CollectionCard
                                    images={collection.media.slice(0, 3).map((m) => m.thumbnail).filter(Boolean) as string[]}
            ...
            {services.length > 0 && (
                <section>
                    <SectionHeader
                        title="Services"
                        href={services.length > 1 ? `/artists/${username}/services` : undefined}
                    />
