...
export function SupportForm({
  ...
  const { handleSubmit, isPending, inputErrors, errors, success } = useHandleAction({
    action: (formData) => {
  ...
  return (
    <FormComponent.Container>
      <FormComponent.Form ref={formRef} onSubmit={handleSubmit} className='gap-4'>
        ...
        <div className='space-y-1 w-full'>
          <Label htmlFor='support-subject' className='block text-xs tracking-wide text-text-muted'>
            {t('formContainer.subjectLabel')} <span className='span-label text-red-500'>*</span>
          </Label>
          <select
            id='support-subject'
            name='subject'
          ...
          {inputErrors?.subject ? <p className='text-xs text-red-500 mt-1'>{inputErrors.subject}</p> : null}
        </div>
        ...
        <FormComponent.SubmitButton isPending={isPending} success={success} className='h-12'>
