...
const EmptyState = ({ displayName }: { displayName: string }) => (
  <div className="mx-auto flex w-full max-w-xl flex-col items-center gap-5 px-6 text-center">
    ...
    <ArtistContactDialog>
      <button
        type="button"
        className="group inline-flex min-h-11 cursor-pointer items-center gap-2.5 rounded-sm border border-border/50 px-7 py-3 text-xs tracking-[0.15em] text-text-muted uppercase transition-all duration-300 hover:border-text/30 hover:text-text"
      >
        <Mail
