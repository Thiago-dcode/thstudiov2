...
import { Service, CreateServiceInputWithFile, UpdateServiceInputWithFile } from "@repo/common-lib/types/service";
...
type ServiceActionInput = Partial<CreateServiceInputWithFile> & Partial<UpdateServiceInputWithFile>;
...
export const createOrUpdateServiceAction = async (
    ...
    if (rawData.slug && currentService?.slug !== rawData.slug) {
        const slugExistsResponse = await userServiceService.slugExists(userAuth.username, rawData.slug);
