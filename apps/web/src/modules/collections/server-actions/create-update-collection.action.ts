...
import { Collection, CreateCollectionInput, UpdateCollectionInput } from "@repo/common-lib/types/collection";
...
import { createCollectionSchema, updateCollectionSchema } from "../schemas/collection-schemas";
import { userSession } from "@/modules/auth/server-actions/user-session.action";

type CollectionActionInput = Partial<CreateCollectionInput> & Partial<UpdateCollectionInput>;
...
export const createOrUpdateCollectionAction = async (
    ...
    if (rawData.slug && rawData.user_id && currentCollection?.slug !== rawData.slug) {
        const slugExistsResponse = await userCollectionService.slugExists(userAuth.username, rawData.slug);
    ...
    const collection = isUpdate
        ? await collectionService.update(currentCollection.id, rawData as UpdateCollectionInput)
