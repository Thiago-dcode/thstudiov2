...
export const SelectCollectionDrawer = ({ userId, collectionsSelected, onSelect }: { userId: number, collectionsSelected: Record<number, unknown>, onSelect: (collection: Collection) => void }) => {
    const [open, setOpen] = useState(false);
    const [collections, setCollections] = useState<Collection[]>([]);
    const collectionMap = useRef(new Map<number, Collection>());
    const firstFetchDone = useRef(false);
    const collectionsSelectedLength = useMemo(() => Object.keys(collectionsSelected).length, [collectionsSelected]);
                                        ...
                                        <div className="absolute bottom-0 inset-x-0 bg-linear-to-t from-black/60 to-transparent px-2.5 pb-2 pt-6">
                                            ...
                                            {c.media?.length > 0 && (
                                                <p className="text-[10px] text-white/70">
                                                    {c.media.length} item{c.media.length !== 1 ? 's' : ''}
                                                </p>
                                            )}
                                        </div>
