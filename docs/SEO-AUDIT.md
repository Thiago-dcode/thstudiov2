...
**One-line state of the world:** the per-entity + technical foundation is **built and verified**; the four original critical blockers are **closed**, and incomplete artist profiles no longer leak out as share/index-ready (§B.7). What remains: the **discovery/directory layer** (§D) — the real growth engine, deliberately deferred until there are artists to fill it — plus the §0 pre-launch gate. Production is **one build behind** (§B.7 written and verified locally, not yet deployed) and still serves the intentional `Disallow: /` + `X-Robots-Tag: noindex` from nginx. §0.1 and §0.2 are verified clean on the droplet, so the launch path is: **deploy → verify §0.3 → drop the two nginx blocks → register GSC/Bing and submit the sitemaps.**
...
- [x] 🚀 **`APP_URL`** = the canonical production origin (e.g. `https://a11studio.com`, **no trailing slash**). Feeds `metadataBase`, every `<link rel="canonical">`, all hreflang alternates, OG/Twitter URLs, JSON-LD `url`/`@id`, and the `Sitemap:`/`Host:` lines in `robots.txt`. A wrong/localhost value = canonicals & OG point at the wrong host and Google discards them. (Schema requires it, but nothing validates it's the *right* host — you must.) ✅ `https://a11studio.com`, no trailing slash, `NEXT_PUBLIC_APP_URL` matches.
...
- [ ] 🚀 **Shard ids resolve to the right kind** — verify after the next deploy that `/sitemap/1.xml` is the **artists** shard and `/sitemap/51.xml` the portfolios shard, with **no URL appearing in two shards**. See §A.2a for the failure this guards against.
