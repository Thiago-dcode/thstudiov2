...
- [ ] 🚀 **An artist shard exists** — on 2026-08-11 there was **no** artists shard at all: the only non-support account is role `ADMIN`, and the sitemap predicate accepted `ARTIST` only, so `counts.artists = 0`. `findAllArtists` (the public directory) never filtered on role, so that profile was browsable and linkable but absent from the sitemap. Fixed by widening the predicate to `PUBLIC_PROFILE_ROLES` (`ARTIST`, `ADMIN`) — confirm `/sitemap/1.xml` is the **profile** shard after deploy.
